...
@dataclass
class VAPORConfig(TrainingArguments):
    r"""
    Configuration class for the [`VAPORTrainer`].

    This class includes only the parameters that are specific to VAPOR training. For a full list of training arguments,
    ...
    Parameters:
        ...
        model_init_kwargs (`str`, `dict[str, Any]` or `None`, *optional*, defaults to `None`):
            ...
            argument of the [`VAPORTrainer`] is provided as a string.
        disable_dropout (`bool`, *optional*, defaults to `False`):
            Whether to disable dropout in the model. This is useful for training with a reference model, as it prevents
            the model from generating different logprobs for the same input.

        > Parameters specifically for VAPOR
        verifiable_term_split (`Tuple[str]`, *optional*, defaults to `("<REASONING>", "</REASONING>")`):
            A tuple containing the start and end tags for the verifiable section.
        preference_term_split (`Tuple[str]`, *optional*, defaults to `("<RESPONSE>", "</RESPONSE>")`):
            A tuple containing the start and end tags for the preference response.
        preference_temperature (`float`, *optional*, defaults to 0.1):
            The temperature parameter to control the strength of the preference signal.
    ...
    # Parameters specifically for VAPOR
    verifiable_term_split: tuple[str] = field(
        default=("<REASONING>", "</REASONING>"),
        metadata={
            "help": "A tuple containing the start and end tags for the verifiable section."
        },
    )
    preference_term_split: tuple[str] = field(
        default=("<RESPONSE>", "</RESPONSE>"),
        metadata={
            "help": "A tuple containing the start and end tags for the preference response."
        },
    )
    preference_temperature: float = field(
        default=0.1,
        metadata={
            "help": "The temperature parameter to control the strength of the preference signal."
        },
    )
    ...
    def __post_init__(self):
        ...
        self.scale_rewards = {True: "group", False: "none"}.get(
            self.scale_rewards, self.scale_rewards
        )
        ...
        if self.do_eval and self.eval_strategy != "no":
            # Just ensure the value is divisible by the global batch size
            if (
                self.per_device_eval_batch_size * num_processes
